...
### =  ( 4) Doesn't use DArrays, faster for medium size matrices.
### =----------------------------------------------------------------------
### = SUBFUNCTIONS:
...
function compute_svd_shared(m::Integer,n::Integer,A::AbstractMatrix,k::Integer,l::Integer,N::Integer,tol::Real,pro::Bool)
...
function compute_B(Bx::AbstractMatrix, A::AbstractMatrix, k::Integer)
   ...
   ### Do a local matmul on each processor (@parallel is 3x faster than mapreduce)
   # By = mapreduce( fetch, hcat, { @spawn DGEMV('T', A, Bx[:,i]) for i in 1:k } )
...
function compute_G(X::AbstractMatrix, A::AbstractMatrix, p::Integer)
   ...
   ### Do a local matmul on each processor (@parallel is 3x faster than mapreduce)
   # Y = mapreduce( fetch, hcat, { @spawn DGEMV('T', A, X[:,i]) for i in 1:p } )
